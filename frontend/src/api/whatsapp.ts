...
export async function createWhatsappAccount(label: string) {
  const { data } = await api.post<{ data: WhatsappAccount; qrcode: Record<string, unknown> | null }>('/whatsapp/accounts', { label });
  return data;
}
...
export async function updateWhatsappAccount(id: number, payload: { label?: string; is_active?: boolean }) {
